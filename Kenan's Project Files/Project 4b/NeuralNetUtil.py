...
def getNNParcelData(fileString="datasets/parcel-training.txt", limit=100000):
    ...
    lineNum = 0
    for line in data:
        # vector attributes = [omconsign_scinbound, scinbound_lhdepart, 
        #                      lhdepart_lharrive, ccin_ccout, ccout_signed]
        inVec = [0,0,0,0,0] # add an extra 0?
        outVec = [0,0,0,0,0]
        count = 0
        for val in line.split(','):
            if count == 5: # number of legs in journey
                outVec[int(val)] = 1
            else:
                inVec[count] = int(val)
            count = count + 1
...
def buildExamplesFromCarData(size=200):
    ...
    carDataTestList = [carDataTrainList.pop(random.randint(0,tests+size-t-1)) for t in range(tests)]
    return carDataTrainList, carDataTestList

# necessary?
def buildExamplesFromParcelData(size=10000):
    '''
    build Neural-network friendly data struct
    ...
    '''
    if (size != 10000):
        parcelDataTrainList = getNNParcelData()
        parcelDataTestList = getNNParcelData()
    else:
        parcelDataTrainList = getNNParcelData()
        parcelDataTestList = getNNParcelData()
...
def getNNVotesData(fileString ="datasets/votes.data.txt", limit=100000 ):
    """
    returns limit # of examples from file passed as string
    """
    examples=[]
    attrValues={}
    data = open(fileString)
    attrs = ['handicap','water','adoption','physician','el-salvador','religious', 'anti-satellite', 'nicaragua',
           'missile', 'immigration', 'synfuels', 'education', 'superfund', 'crime', 'exports', 'export-administration']
