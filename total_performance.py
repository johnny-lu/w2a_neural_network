...
for i in STATUSES:
    ...
    median = np.median(series['total_time'])     
    training_in = np.array(training[STATUSES[:number]])
    training_out = [[i] for i in training['total_time'].tolist()]
    ...
    filename = 'my_model%d' % number

    try:
        model = load_model(filename)
        print('Model Loaded')
        model.summary()
        #model.fit(training_in,training_out,epochs=35, batch_size=100)
        ...
        model.add(Dense(8, input_dim= number, activation = 'relu')) # input layer (number of neurons, number of inputs, activation function)
        ...
        model.fit(training_in,training_out,epochs=50, batch_size=100)
