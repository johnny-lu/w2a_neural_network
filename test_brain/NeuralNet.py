...
class Perceptron(object):
    ...
    def getWeightedSum(self, inActs):
        ...
        """
        inActs = [float(i)
                  for i in inActs]  # makes sure everything in list is a number
        return sum([inActs * inWt for inActs,
                    inWt in zip(inActs, self.weights)])
...
def buildNeuralNet(
        examples,  # data in
        numOut=1,  # number of output perceptrons
        alpha=0.1,
    ...
    Args:
        examples (list<tuple<list,list>>): A list of training examples
        ...
        startNNet (NeuralNet):                   A NeuralNet to train, or none for a new NeuralNet
                                                 to be trained from random weights.
    Returns
       NeuralNet

       The trained Neural Network once the weight modification reached the
       threshold, or the iteration exceeds the maximum iteration.
    """
    numIn = len(examples[0][0])
    ...
    print(
        "Starting training at time %s with %d inputs, %s hidden layers, size of training set %d." %
        (str(time), numIn, str(hiddenLayerList), len(examples)))
    layerList = [numIn] + hiddenLayerList + [numOut]
    nnet = NeuralNet(layerList)
    if startNNet is not None:
        nnet = startNNet
    ...
    # else :
    #    print '.',
    trainError, weightMod = nnet.backPropLearning(examples, alpha)
...
def predict(
    ...
    # examples is a tuple of inputs and outputs to compare against
...
'''
Used to be buildNeuralNet. Constructs a NeuralNet if one is not passed, trains
it using the given training data, and self-evaluates its accuracy using given
test data.
'''


def buildNeuralNetComplete(
        examples,
        alpha=0.1,
        weightChangeThreshold=0.00008,
        hiddenLayerList=[1],
